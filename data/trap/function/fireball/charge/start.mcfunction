...
execute anchored eyes positioned ^ ^ ^1.5 run scoreboard players operation @n[tag=Fireball,distance=..0.01] ParentID = @s ID
execute anchored eyes positioned ^ ^ ^1.5 run scoreboard players set @n[tag=Fireball,distance=..0.01] MagicTime 30
execute anchored eyes positioned ^ ^ ^1.5 run tp @n[tag=Fireball,distance=..0.01] ^ ^ ^ ~ ~
